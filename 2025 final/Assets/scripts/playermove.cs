...
public class playermove : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        walljumpL = Physics.Raycast(transform.position, Vector3.left, playerheight * 1f + 0.5f, whatiswall);
        walljumpR = Physics.Raycast(transform.position, Vector3.right, playerheight * 1f + 0.5f, whatiswall);
        //drag stuffs 
        if (grounded)
        {
            rb.drag = groundDrag;
    ...
    private void input()
    {
        ...
        if (Input.GetKey(jumpkey) && canjump && grounded || walljumpL || walljumpR)
        {
            canjump = false;
            jump();
